...
export async function registerRoutes(
  ...
  app.post(api.analysis.upload.path, async (req, res) => {
    try {
      const input = api.analysis.upload.input.parse(req.body);
      ...
      res.status(500).json({ message: "Échec de l'analyse du fichier GPX" });
    }
  });
